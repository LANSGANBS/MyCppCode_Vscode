...
void solve() {
  ...
  vector<Team> t(n);
  for (int i = 0; i < n; i++) {
    cin >> t[i].school >> t[i].team >> t[i].id;
  }
  int bs;
  cin >> bs;
  ...
  int quotaB = m * 3 / 10;
  int quotaC = m / 10;
  vector<Team> resA, resB, resC;
  unordered_map<string, int> ald;
  for (auto &tm : t) {
    if (ald[tm.school] >= 3) continue;
    if (quotaA > 0) {
      resA.push_back(tm);
      quotaA--;
      ald[tm.school]++;
    } else if (st.count(tm.school) && quotaB > 0) {
      resB.push_back(tm);
      quotaB--;
      ald[tm.school]++;
    } else if (Cset.count(tm.school) && quotaC > 0) {
      resC.push_back(tm);
      quotaC--;
      ald[tm.school]++;
    }
  ...
  cout << resA.size() << endl;
  for (auto &x : resA) {
    cout << x.school << " " << x.team << " " << x.id << endl;
  }
  cout << resB.size() << endl;
  for (auto &x : resB) {
    cout << x.school << " " << x.team << " " << x.id << endl;
