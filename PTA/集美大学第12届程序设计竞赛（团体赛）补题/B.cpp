...
void solve() {
  ...
  vector<PR<int, int>> pos;
  vector<PR<int, int>> neg;
  ...
  sort(
      pos.begin(), pos.end(),
      [](const PR<int, int> &x, const PR<int, int> &y) { return x.sc < y.sc; });
  int count = 0;
  for (auto &p : pos) {
    int a = p.fr, b = p.sc;
    h += (a - b);
  ...
  sort(neg.begin(), neg.end(),
       [](const PR<int, int> &x, const PR<int, int> &y) {
