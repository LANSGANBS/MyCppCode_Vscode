...
int lucky(int l, int r, int k, int &cnt) {
  if (r - l + 1 < k) {
    cnt = 0;
    return 0;
  }
  ...
  int leftcnt = 0;
  if (!((r - l + 1) & 1)) {
    int l1 = lucky(l, mid, k, leftcnt);
    cnt = 2 * leftcnt;
    return 2 * l1 + mid * leftcnt;
  } else {
    int l1 = lucky(l, mid - 1, k, leftcnt);
    cnt = 2 * leftcnt + 1;
    return mid + 2 * l1 + mid * leftcnt;
  }
}
