...
template <typename CharType, typename DataType>
bool sstree<CharType, DataType>::insert(const std::basic_string<CharType> &str,
                                        const DataType &data) {
  iterator ptr_l = &tree;
  ...
  if (ptr_l->data) return false;
  ptr_l->data = new DataType(data);
  return true;
}
template <typename CharType, typename DataType>
typename sstree<CharType, DataType>::iterator
sstree<CharType, DataType>::search(const std::basic_string<CharType> &str) {
  typename std::map<CharType, tree_in>::iterator ptr_r;
  iterator ptr_l = ptr;
  for (const CharType &ch : str) {
    ptr_r = ptr_l->child.find(ch);
    if (ptr_r == ptr_l->child.end()) return nullptr;
    ptr_l = &(ptr_r->second);
  }
  return ptr_l;
}
...
bool sstree<CharType, DataType>::erase(const std::basic_string<CharType> &str) {
  ...
  for (size_t pos = 0; pos < str.size(); ++pos) {
    ptr_r = ptr_l->child.find(str[pos]);
    if (ptr_r == ptr_l->child.end()) return false;
...
enum class vartype { CONSTVAR, FREEVAR };
...
struct evaluator {
  ...
  evaluator(std::function<bool(const StringType &, size_t &, epre<DataType> &)>
                consts_,
            ...
            std::shared_ptr<sstree<CharType, func<DataType>>> suf_ops = nullptr)
      : consts(consts_),
        vars(vars_ ? vars_
                   : std::make_shared<sstree<CharType, var<DataType>>>()),
        funcs(funcs_ ? funcs_
                     : std::make_shared<sstree<CharType, func<DataType>>>()),
        prefix_ops(pre_ops
                       ? pre_ops
                       : std::make_shared<sstree<CharType, func<DataType>>>()),
        infix_ops(in_ops
...
template <typename CharType, typename DataType>
size_t evaluator<CharType, DataType>::parse(epre<DataType> &expr,
  ...
  while (pos < str.size()) {
    ...
    if (expecting_operand) {
      ...
      typename sstree<CharType, func<DataType>>::iterator it =
          prefix_ops->find(str[pos]);
      if (it) {
        size_t start = pos;
      ...
      typename sstree<CharType, var<DataType>>::iterator var_it =
          vars->find(str[pos]);
      if (var_it) {
        size_t start = pos;
        vars->setptr(var_it);
    ...
    if (!expecting_operand) {
      ...
      typename sstree<CharType, func<DataType>>::iterator op_it =
          infix_ops->find(str[pos]);
      if (op_it) {
        size_t start = pos;
  ...
  for (char ch : expr.index) {
    switch (ch) {
      case 'f': {
        ...
        if (stack.size() < f->size) throw std::runtime_error("Stack underflow");
